...
void foreground_search(DynamicIndex<T> &index, QueryBundle<T> &queries, uint32_t k, uint32_t L, uint32_t rounds,
  ...
  for (uint32_t r = 0; r < rounds; ++r) {
    ...
    if (queries.selector) {
      std::vector<pipeann::Attributes> one_attrs{queries.attrs[qid]};
      index.search(queries.data + static_cast<size_t>(qid) * queries.dim, 1, k, L, one_ids.data(), one_dists.data(),
                   queries.selector.get(), one_attrs);
    } else {
      index.search(queries.data + static_cast<size_t>(qid) * queries.dim, 1, k, L, one_ids.data(), one_dists.data());
    }
...
int run_dynamic(int argc, char **argv) {
  ...
  live_indexes.label = index.load_attr_index_from_file(0, label_index, "label");
  live_indexes.range = index.load_attr_index_from_file(1, range_index, "range");
  if (!label_config.empty() && label_config != "null") {
    auto loaded = load_selector_from_live_indexes(label_config, live_indexes);
  ...
  for (uint32_t cycle = 1; cycle <= cycles; ++cycle) {
    ...
    for (uint32_t i = 0; i < count; ++i) {
      attrs[i] = attrs_for_tag(begin + i, rank);
    }

...
while IFS=, read -r selector_id selector_type target_selectivity candidate_count query_file label_config; do
  ...
  if [[ "${selector_type}" == "match_all" ]]; then
    "${UTIL_BIN_DIR}/compute_groundtruth" float l2 "${WORK_DIR}/data/base.bin" "${WORK_DIR}/data/query.bin" 10 "${GT}" null null
    LABEL_ARG="null"
  else
    "${UTIL_BIN_DIR}/compute_groundtruth" float l2 "${WORK_DIR}/data/base.bin" "${WORK_DIR}/data/query.bin" 10 "${GT}" null "${label_config}"
...
for cycle in 1 2; do
  ...
  while IFS=, read -r selector_id selector_type target_selectivity candidate_count query_file label_config; do
    ...
    if [[ "${selector_type}" == "match_all" ]]; then
      "${UTIL_BIN_DIR}/compute_groundtruth" float l2 "${CYCLE_BIN}" "${WORK_DIR}/data/query.bin" 10 "${GT}" null null
    else
      "${UTIL_BIN_DIR}/compute_groundtruth" float l2 "${CYCLE_BIN}" "${WORK_DIR}/data/query.bin" 10 "${GT}" null "${label_config}"
    fi
  done < "${WORK_DIR}/labels/selector_manifest.csv"
done
